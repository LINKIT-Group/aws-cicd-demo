...
## BACKEND SETUP ##
# Used for dynamic task definition.
# source: https://github.com/cloudposse
...
module "ecs_backend" {
  ...
  name                = "${var.repository_base_name}-${local.application[1]}"
  environment         = "${local.environment[0]}"
  region              = "${var.region}"
  vpc_id              = "${module.vpc.vpc_id}"
  availability_zones  = "${module.vpc.azs}"
  repository_name     = "${var.repository_base_name}-${local.application[1]}/${local.environment[0]}"
  ...
  task_cpu            = "1024"
  task_memory         = "2048"
  container_name  = "${var.service_name}-${local.application[1]}"
  container_port  = "80"
...
module "code_pipeline_backend" {
  ...
  codecommit_repository_name  = "${var.repository_base_name}-${random_string.name.result}"
